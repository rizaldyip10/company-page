    ...
    return (
        <div className='w-full flex justify-center items-center bg-[#37393F] py-36 px-10 lg:px-20'>
            <div className='w-full flex flex-col items-center gap-12'>
                <AnimationWrapper classname='w-full flex flex-col gap-4 lg:gap-0 lg:flex-row justify-between items-center xl:px-52'>
                ...
                <div className='w-full flex flex-wrap justify-center gap-7'>
                    {
                        offers.map((item, index) => (
                            <AnimationWrapper key={index} transition={{ duration: .7, delay: index * .1}}>
                                <OfferCard icon={item.icon} title={item.title} content={item.content} />
                            </AnimationWrapper>
                        ))
                    }
                </div>
